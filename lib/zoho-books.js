...
export function paySources() {
  // Order matters: the FIRST source is the default everywhere salaries are
  // paid (Adama 3 Aug: staff salaries default to Access Bank, not Wave).
  return [
...
// Every Salaries expense in a month, whoever it was paid to — lets the payroll
// run discover past staff and pre-Pulse payments in one query (Adama 9 Jul).
export async function salaryExpensesForMonth(ym) {
  const start = `${ym}-01`
  ...
  const out = []
  while (page <= 10) {
    const d = await zohoGet(`/books/v3/expenses?organization_id=${orgId()}&date_start=${start}&date_end=${end}&page=${page}&per_page=200`)
    const items = Array.isArray(d.expenses) ? d.expenses : []
    for (const e of items) {
      if (String(e.account_id) === String(salaryAccountId()) || /salar|wage/i.test(e.account_name || '')) {
        out.push({ expense_id: e.expense_id, total: e.total, date: e.date, vendor_id: e.vendor_id || null, vendor_name: e.vendor_name || '', description: e.description || '' })
      }
    }
    if (!d.page_context?.has_more_page) break
    page++
...
export async function recordSalaryPayment({ name, salary = 0, bonus = 0, paySourceKey, date, period, label = '', autoCreateVendor = true, force = false, dryRun = false }) {
  ...
  const payload = {
    ...
    date,
    amount: total,
    description: `Payroll ${ym} — ${name}${labelNote}${bonusNote}. Paid via ${src.label}.`,
  }
...
export async function updateSalaryExpense(expenseId, { name, vendorId, salary = 0, bonus = 0, paySourceKey, date, period, label = '' }) {
  if (!salaryAccountId()) throw new Error('ZOHO_SALARY_ACCOUNT_ID not set')
  const src = paySourceById(paySourceKey)
  if (!src) throw new Error('Unknown pay source: ' + paySourceKey)
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date || '')) throw new Error('Valid date (YYYY-MM-DD) required')
