...
const perfColor = p => p >= 80 ? 'text-emerald-600' : p >= 50 ? 'text-amber-600' : p > 0 ? 'text-red-600' : 'text-[var(--color-ink-faint)]';
const perfBg = p => p >= 80 ? 'bg-emerald-500' : p >= 50 ? 'bg-amber-500' : p > 0 ? 'bg-red-500' : 'bg-[var(--color-ink-faint)]';
const perfLabel = p => p >= 80 ? 'On Track' : p >= 50 ? 'Needs Attention' : p > 0 ? 'Underperforming' : 'New';
const statusBadge = s => ({ active: 'bg-emerald-100 text-emerald-700', maternity: 'bg-purple-100 text-purple-700', probation: 'bg-amber-100 text-amber-700', training: 'bg-orange-100 text-orange-700' })[s] || 'bg-[var(--color-fill)] text-[var(--color-ink-soft)]';
  ...
  return (
    <div>
      <div className="mb-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-[27px] font-semibold text-[var(--color-ink)]">{title}</h1>
          ...
          {weekOverview.map((w) => {
            ...
            <div key={w.lead.username} className="bg-white rounded-lg border border-[var(--color-line)] p-4">
              ...
              <div className="space-y-1.5">
                {w.focus.length === 0 && <p className="text-sm text-[var(--color-ink-faint)]">Nothing behind target.</p>}
                {w.focus.map((f, i) => (
                  <div key={f.key} className="text-sm text-[var(--color-ink-soft)]">
                    ...
                    {f.note && <p className="mt-0.5 rounded bg-[var(--color-fill)] px-2 py-1 text-xs text-[var(--color-ink-soft)]"><span className="font-semibold">His comment:</span> {f.note}</p>}
                  </div>
                ))}
                {(w.adamaOverdue || []).length > 0 && (
                  <div className="rounded-lg bg-red-50 px-2.5 py-2 text-xs font-semibold text-red-700">
                ...
                {w.other && (w.other.title || w.other.note) && (
                  <div className="text-sm text-[var(--color-ink-soft)]">
                    ...
                    {w.other.note && <p className="mt-0.5 rounded bg-[var(--color-fill)] px-2 py-1 text-xs text-[var(--color-ink-soft)]"><span className="font-semibold">His comment:</span> {w.other.note}</p>}
                  </div>
                )}
              </div>
              ...
              <div className="mt-3 flex gap-1.5 border-t border-[var(--color-line-soft)] pt-2">
                <input value={d.title} onChange={(e) => setAssignDraft((s) => ({ ...s, [w.lead.username]: { ...d, title: e.target.value } }))} onKeyDown={(e) => e.key === 'Enter' && sendAssignment(w.lead.username)} placeholder={`Send ${w.lead.name.split(' ')[0]} a task…`} className="min-w-0 flex-1 rounded-lg border border-[var(--color-line)] px-2.5 py-1.5 text-xs" />
                <input type="date" value={d.due || ''} onChange={(e) => setAssignDraft((s) => ({ ...s, [w.lead.username]: { ...d, due: e.target.value } }))} className="rounded-lg border border-[var(--color-line)] px-2 py-1.5 text-xs text-[var(--color-ink-soft)]" />
                <button onClick={() => sendAssignment(w.lead.username)} className="rounded-lg bg-[var(--color-ink)] px-2.5 py-1.5 text-xs font-bold text-white">Send</button>
              </div>
            </div>
          );})}
        </div>
      ...
      {showOverview && alerts.length > 0 && (
        <div className="mb-6 space-y-2">
          <p className="text-xs font-bold text-[var(--color-ink-faint)] uppercase tracking-wide flex items-center gap-2"><AlertTriangle size={14} className="text-red-500" /> Attention Required</p>
          {alerts.map((a, i) => (
            <div key={i} className={`flex items-center gap-3 p-3 rounded-lg border ${a.type === 'danger' ? 'bg-red-50 border-red-200' : a.type === 'warning' ? 'bg-amber-50 border-amber-200' : 'bg-blue-50 border-blue-200'}`}>
              ...
              <div className="flex-1">
                ...
                {a.detail && <p className="text-xs text-[var(--color-ink-soft)]">{a.detail}</p>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex gap-1 mb-6 bg-[var(--color-fill)] p-1 rounded-lg w-fit flex-wrap">
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${tab === t.id ? 'bg-white text-[var(--color-ink)]' : 'text-[var(--color-ink-soft)] hover:text-[var(--color-ink)]'}`}>{t.label}</button>
        ))}
      </div>
      )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {contractDeadlines.filter(c => c.status === 'probation' || c.status === 'training').map((c, i) => (
                <div key={i} onClick={() => openProfile(c.name)} className="p-4 border border-[var(--color-line)] rounded-lg cursor-pointer transition-shadow">
                  <div className="flex items-center justify-between mb-2">
                    <div><p className="font-medium text-[var(--color-ink)] text-sm">{c.name}</p><p className="text-xs text-[var(--color-ink-soft)]">{c.role} — {c.contract}</p></div>
                  ...
                  <p className="text-xs text-[var(--color-ink-soft)] mt-2">{c.nextActionNote}</p>
      ...
      {tab === 'performance' && (
        <div>
          <div className="flex items-baseline justify-between mb-6">
            <div>
              <h3 className="text-lg font-semibold text-[var(--color-ink)]">Performance</h3>
              <p className="text-sm text-[var(--color-ink-soft)] mt-1">Who is delivering and who is not</p>
            </div>
            <p className="text-xs text-[var(--color-ink-faint)]">{team.filter(t => t.status !== 'maternity' && !archivedNames.has(t.name)).length} people</p>
          </div>
                ...
                <div key={i} onClick={() => openProfile(t.name)} className="bg-white rounded-lg border border-[var(--color-line-soft)] p-6 cursor-pointer hover:border-[var(--color-line)] transition-all">
                  <div className="flex items-start gap-4 mb-5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[var(--color-ink-soft)] truncate">{t.role}</p>
                  ...
                  {t.kpi && (
                    <div className="mt-4">
                      ...
                      <p className="text-xs text-[var(--color-ink-soft)] line-clamp-2">{t.kpi}</p>
                    </div>
                  )}
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="text-lg font-semibold text-[var(--color-ink)]">KPI Settings</h3>
                </div>
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-[var(--color-ink)] hover:bg-[var(--color-ink)] text-white text-sm font-medium rounded-full"
              >
            ...
            <div className="bg-white rounded-lg border-2 border-[var(--color-ink)] p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setKpiForm({ ...kpiForm, scope: 'role', agent: '' })}
                      className={`px-4 py-2 rounded-full text-sm font-medium ${kpiForm.scope === 'role' ? 'bg-[var(--color-ink)] text-white' : 'bg-white text-[var(--color-ink-soft)] border border-[var(--color-line)] hover:border-[var(--color-ink-faint)]'}`}>
                      A role (e.g. Sales Agent)
                    </button>
                    <button onClick={() => setKpiForm({ ...kpiForm, scope: 'agent', role: '' })}
                      className={`px-4 py-2 rounded-full text-sm font-medium ${kpiForm.scope === 'agent' ? 'bg-[var(--color-ink)] text-white' : 'bg-white text-[var(--color-ink-soft)] border border-[var(--color-line)] hover:border-[var(--color-ink-faint)]'}`}>
                      A specific person
                    </button>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <label htmlFor="rule-active" className="text-sm text-[var(--color-ink-soft)]">Active — apply this rule in performance resolution</label>
                </div>
                ...
                <div className="flex justify-end gap-2 pt-2 border-t border-[var(--color-line-soft)]">
                  <button onClick={() => { setKpiEditing(false); setKpiForm(blankRule); }}
                    className="px-4 py-2.5 text-sm text-[var(--color-ink-soft)] hover:text-[var(--color-ink)]">Cancel</button>
                  <button disabled={kpiSaving || (kpiForm.scope === 'role' ? !kpiForm.role : !kpiForm.agent)}
                    onClick={saveKpiRule}
                    className="px-6 py-2.5 bg-[var(--color-ink)] hover:bg-[var(--color-ink)] text-white text-sm font-medium rounded-full disabled:bg-[var(--color-ink-faint)]">
          ...
          <div className="bg-white rounded-lg border border-[var(--color-line-soft)] overflow-hidden">
            {kpiLoading ? (
              ...
              <div className="p-10 text-center">
                <p className="text-sm text-[var(--color-ink-soft)]">{kpiRules.length === 0 ? 'No KPI rules yet. Click "Add KPI rule" above to set your first one.' : 'No rules match this filter.'}</p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[var(--color-line)] bg-[var(--color-fill)]">
                      <th className="text-left py-3 px-4 text-[var(--color-ink-soft)] font-medium text-xs">Scope</th>
                      <th className="text-left py-3 px-4 text-[var(--color-ink-soft)] font-medium text-xs">Target</th>
                      <th className="text-left py-3 px-4 text-[var(--color-ink-soft)] font-medium text-xs">Period</th>
                      <th className="text-center py-3 px-4 text-[var(--color-ink-soft)] font-medium text-xs">Personal</th>
                      <th className="text-center py-3 px-4 text-[var(--color-ink-soft)] font-medium text-xs">Team</th>
                      <th className="text-left py-3 px-4 text-[var(--color-ink-soft)] font-medium text-xs">Weekly</th>
                      <th className="text-left py-3 px-4 text-[var(--color-ink-soft)] font-medium text-xs">KPI</th>
                      <th className="text-center py-3 px-4 text-[var(--color-ink-soft)] font-medium text-xs">Active</th>
                      ...
                      return (
                        <tr key={r.id} className={`border-b border-[var(--color-line-soft)] ${!r.active ? 'opacity-50' : ''}`}>
                          ...
                          <td className="py-3 px-4 text-center">
                            <span className={`w-2 h-2 rounded-full inline-block ${r.active ? 'bg-emerald-500' : 'bg-[var(--color-ink-faint)]'}`} />
                          </td>
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-sm text-[var(--color-ink-soft)]">{p.role}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${statusBadge(p.status)}`}>{p.status}</span>
                    <span className="text-sm text-[var(--color-ink-faint)] flex items-center gap-1">View profile <ChevronDown size={15} className="-rotate-90" /></span>
                  </div>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mt-4 pt-4 border-t border-[var(--color-line-soft)]">
                  <div><p className="text-[10px] uppercase tracking-wider font-bold text-[var(--color-ink-faint)]">Joined</p><p className="text-sm text-[var(--color-ink)] mt-0.5">{p.joined || '—'}</p></div>
                  <div><p className="text-[10px] uppercase tracking-wider font-bold text-[var(--color-ink-faint)]">Department</p><p className="text-sm text-[var(--color-ink)] mt-0.5">{p.type || '—'}</p></div>
                  <div><p className="text-[10px] uppercase tracking-wider font-bold text-[var(--color-ink-faint)]">Salary</p><p className="text-sm text-[var(--color-ink)] mt-0.5">D{(payMap[p.name]?.base || 0).toLocaleString()}</p></div>
                  <div><p className="text-[10px] uppercase tracking-wider font-bold text-[var(--color-ink-faint)]">Commission</p><p className="text-sm text-[var(--color-ink)] mt-0.5">{payMap[p.name]?.commission > 0 ? `Up to D${payMap[p.name].commission.toLocaleString()}` : '—'}</p></div>
                  <div><p className="text-[10px] uppercase tracking-wider font-bold text-[var(--color-ink-faint)]">Contract ends</p><p className={`text-sm mt-0.5 ${daysLeft !== null && daysLeft <= 30 ? 'text-red-600' : daysLeft !== null && daysLeft <= 90 ? 'text-amber-600' : 'text-[var(--color-ink)]'}`}>{ends}</p></div>
                  <div><p className="text-[10px] uppercase tracking-wider font-bold text-[var(--color-ink-faint)]">Warnings</p><p className={`text-sm mt-0.5 ${warns > 0 ? 'text-red-600 font-medium' : 'text-[var(--color-ink)]'}`}>{warns}</p></div>
                </div>
              </div>
            );
          })}
        ...
        <div className="space-y-6">
          ...
          {paySec === 'run' && canSeePayDetail && payRun && !payRun.error && (
            <div className="bg-white rounded-lg border border-[var(--color-line)] p-6">
              <div className="flex items-start justify-between mb-1 flex-wrap gap-2">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <input
                      ...
                      className="text-sm border border-[var(--color-line)] rounded-lg px-3 py-1.5 text-[var(--color-ink-soft)]"
                ...
                <div className="flex items-center gap-2">
                  <button type="button" onClick={openOneOff} className="px-3 py-2 rounded-lg text-sm font-medium bg-white border border-[var(--color-line)] text-[var(--color-ink-soft)] hover:border-[var(--color-line-soft)]0">+ Record a payment</button>
                  {(payRun.people || []).some(p => !p.paid) && (
                    <button type="button" onClick={openBulk} className="px-3 py-2 rounded-lg text-sm font-medium bg-[var(--color-ink)] text-white hover:bg-[var(--color-ink)]">Pay all ({(payRun.people || []).filter(p => !p.paid).length})</button>
                  )}
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead><tr className="border-b border-[var(--color-line)] text-xs uppercase text-[var(--color-ink-soft)]">
                      ...
                      return (
                        <tr key={p.name} className="border-b border-[var(--color-line-soft)]">
                          <td className="px-3 py-2">
                            <p className="text-sm font-medium text-[var(--color-ink)]">{p.name}{p.past && <span className="ml-2 px-1.5 py-0.5 rounded-full bg-[var(--color-fill)] text-[var(--color-ink-soft)] text-[10px] font-semibold uppercase tracking-wide">Past staff</span>}</p>
                            <p className="text-xs text-[var(--color-ink-soft)]">{p.role}</p>
                          </td>
                          {p.paid ? (
                            <>
                              <td className="px-3 py-2 text-right text-sm tabular-nums text-[var(--color-ink-soft)]">D{(p.paid.salary || 0).toLocaleString()}</td>
                              <td className="px-3 py-2 text-right text-sm tabular-nums text-[var(--color-ink-soft)]">{Number(p.paid.bonus) > 0 ? `D${p.paid.bonus.toLocaleString()}` : '—'}</td>
                              <td className="px-3 py-2 text-right text-sm font-bold whitespace-nowrap">D{p.paid.total.toLocaleString()}</td>
                              <td className="px-3 py-2 text-sm text-[var(--color-ink-soft)]">{p.paid.paySource || '—'}</td>
                              <td className="px-3 py-2 text-sm text-emerald-700">{p.paid.label || <span className="text-[var(--color-ink-faint)]">—</span>}</td>
                            </>
                          ) : (
                            <>
                              <td className="px-3 py-2 text-right"><input type="number" value={d.salary} onChange={e => setD({ salary: e.target.value })} className="w-24 text-right border border-[var(--color-line)] rounded px-2 py-1 text-sm" /></td>
                              <td className="px-3 py-2 text-right"><input type="number" value={d.bonus} onChange={e => setD({ bonus: e.target.value })} className="w-24 text-right border border-[var(--color-line)] rounded px-2 py-1 text-sm" /></td>
                              <td className="px-3 py-2 text-right text-sm font-bold whitespace-nowrap">D{total.toLocaleString()}</td>
                              <td className="px-3 py-2">
                                <select value={d.source} onChange={e => setD({ source: e.target.value })} className="border border-[var(--color-line)] rounded px-2 py-1 text-sm">
                                  {(payRun.paySources || []).map(s => <option key={s.key} value={s.key}>{s.label}</option>)}
                                </select>
                              </td>
                              <td className="px-3 py-2"><input value={d.note || ''} onChange={e => setD({ note: e.target.value })} placeholder="optional" className="w-36 border border-[var(--color-line)] rounded px-2 py-1 text-sm" title="What this payment is, if not plain salary — goes to Books and the payslip" /></td>
                            </>
                          )}
                          {!p.paid ? (
                            <td className="px-3 py-2 text-right">
                              <button type="button" onClick={() => startPay(p)} disabled={total <= 0} className={`px-3 py-1.5 rounded-lg text-sm font-medium ${total > 0 ? 'bg-[var(--color-ink)] text-white hover:bg-[var(--color-ink)]' : 'bg-[var(--color-fill)] text-[var(--color-ink-faint)] cursor-not-allowed'}`}>Mark paid</button>
                            </td>
                          ) : (
                    ...
                    return (
                      <tfoot>
                        <tr className="border-t-2 border-[var(--color-line)]">
                          <td className="px-3 py-3 text-sm font-bold text-[var(--color-ink)]">Estimated total</td>
                          <td className="px-3 py-3 text-right text-sm font-bold text-[var(--color-ink)]">D{sal.toLocaleString()}</td>
                          <td className="px-3 py-3 text-right text-sm font-bold text-[var(--color-ink)]">D{bon.toLocaleString()}</td>
                          <td className="px-3 py-3 text-right text-sm font-bold text-[var(--color-ink)]">D{(sal + bon).toLocaleString()}</td>
                          <td colSpan={3}></td>
                        </tr>
                      </tfoot>
                    );
          ...
          <div className="bg-white rounded-lg border border-[var(--color-line)] p-6">
            ...
            <table className="w-full"><thead><tr className="border-b border-[var(--color-line)]">
              ...
              <th className="text-right px-4 py-3 text-xs uppercase text-[var(--color-ink-soft)]">Base</th><th className="text-right px-4 py-3 text-xs uppercase text-[var(--color-ink-soft)]">Commission</th>
              <th className="text-right px-4 py-3 text-xs uppercase text-[var(--color-ink-soft)]">Total</th>
            </tr></thead><tbody>
              {team.filter((p) => !archivedNames.has(p.name)).map((p, i) => { const pay = payMap[p.name] || {}; return <tr key={i} className="border-b border-[var(--color-line-soft)]"><td className="px-4 py-3 text-sm font-medium text-[var(--color-ink)]">{p.name}</td><td className="px-4 py-3 text-sm text-[var(--color-ink-soft)]">{p.role}</td><td className="px-4 py-3 text-sm text-right">D{(pay.base || 0).toLocaleString()}</td><td className="px-4 py-3 text-sm text-right">{pay.commission > 0 ? <span className="text-green-600">Up to D{pay.commission.toLocaleString()}</span> : '—'}</td><td className="px-4 py-3 text-sm font-bold text-right">D{(pay.total || 0).toLocaleString()}</td></tr>; })}
            </tbody><tfoot><tr className="border-t-2 border-[var(--color-line)]"><td colSpan={4} className="px-4 py-3 font-bold">Total</td><td className="px-4 py-3 text-lg font-bold text-right">D{totalPayroll.toLocaleString()}</td></tr></tfoot></table>
          </div>
          )}
          {paySec === 'history' && (
          <div className="bg-white rounded-lg border border-[var(--color-line)] p-6">
            ...
            <div className="flex items-center gap-2 mb-4">
              <select value={payHistYear} onChange={(e) => setPayHistYear(e.target.value)} className="border border-[var(--color-line)] rounded-lg px-3 py-1.5 text-sm bg-white">
                <option value="all">All years</option>
                {histYears.map((y) => <option key={y} value={y}>{y}</option>)}
              </select>
              {canSeePayDetail && (
                <input value={payHistSearch} onChange={(e) => setPayHistSearch(e.target.value)} placeholder="Search a person…" className="flex-1 max-w-xs border border-[var(--color-line)] rounded-lg px-3 py-1.5 text-sm" />
              )}
            </div>
            <div className="space-y-3">{(() => {
              ...
              if (q) {
                ...
                if (!hits.length) return <p className="px-4 py-8 text-sm text-[var(--color-ink-faint)] text-center">No payments match “{payHistSearch}”.</p>;
                return (
                    ...
                    <table className="w-full">
                      ...
                      <tbody>{hits.map((h, i) => (
                        <tr key={i} className="border-b border-[var(--color-line-soft)] last:border-0">
                          <td className="px-4 py-2 text-sm text-[var(--color-ink-soft)] whitespace-nowrap">{h.month}</td>
                          <td className="px-4 py-2 text-sm text-[var(--color-ink)]">{h.name}{h.note && <span className="block text-xs text-[var(--color-ink-faint)]">{h.note}</span>}</td>
                          <td className="px-4 py-2 text-sm text-right font-medium whitespace-nowrap">D{(h.amount || 0).toLocaleString()}</td>
                        </tr>
                      ))}</tbody>
                      <tfoot><tr className="border-t-2 border-[var(--color-line)]"><td className="px-4 py-2 text-sm font-bold" colSpan={2}>{hits.length} payment{hits.length === 1 ? '' : 's'}</td><td className="px-4 py-2 text-sm text-right font-bold">D{total.toLocaleString()}</td></tr></tfoot>
                    </table>
                  </div>
                );
              }
                ...
                <div key={key} className="border-b border-[var(--color-line-soft)] last:border-b-0">
                  ...
                  >
                    <div className="flex-1 min-w-0 flex items-center gap-2 flex-wrap px-3 py-2">
                      ...
                      <span className="text-sm font-medium text-[var(--color-ink)]">{m.month}</span>
                      ...
                      {m.breakdown
                        ? <span className="text-xs text-[var(--color-ink-faint)] truncate hidden md:inline">· {m.breakdown}</span>
                        : m.headcount != null && <span className="text-xs text-[var(--color-ink-faint)]">· {m.headcount} {m.headcount === 1 ? 'person' : 'people'} paid</span>}
                    </div>
                      ...
                      {m.people.map((p, j) => (
                        <div key={j} className="flex items-stretch border-b border-[var(--color-line-soft)]/70">
                          <div className="flex-1 min-w-0 px-3 py-2 pl-[34px]">
                            <span className={`text-sm ${p.unallocated ? 'italic text-[var(--color-ink-soft)]' : 'text-[var(--color-ink-soft)]'}`}>{p.name}</span>
                            {p.note && <span className="block text-xs text-[var(--color-ink-faint)]">{p.note}</span>}
                          </div>
                          <div className="w-36 shrink-0 flex items-center justify-end border-l border-[var(--color-line-soft)] px-3 py-2">
                            <span className="text-sm text-[var(--color-ink-soft)] whitespace-nowrap">D{(p.amount || 0).toLocaleString()}</span>
                          </div>
                        </div>
                      ))}
                      <div className="flex items-stretch border-t border-[var(--color-line)] border-b border-[var(--color-line-soft)] bg-[var(--color-fill)]/50">
                        <div className="flex-1 min-w-0 px-3 py-2 pl-[34px]"><span className="text-sm font-bold text-[var(--color-ink)]">Total</span></div>
                        <div className="w-36 shrink-0 flex items-center justify-end border-l border-[var(--color-line-soft)] px-3 py-2">
                          <span className="text-sm font-bold text-[var(--color-ink)] whitespace-nowrap">D{itemised.toLocaleString()}</span>
                        </div>
                      </div>
                      {!reconciles && (
                ...
                return (
                  <div key={y} className="border border-[var(--color-line)] rounded-lg overflow-hidden">
                    <button type="button" onClick={() => toggleYear(y)} className="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-[var(--color-fill)]">
                      <span className="text-sm font-semibold text-[var(--color-ink)] flex items-center gap-2">
                        <ChevronDown size={15} className={`text-[var(--color-ink-faint)] transition-transform ${open ? '' : '-rotate-90'}`} />{y}
                      </span>
                      <span className="text-xs text-[var(--color-ink-faint)]">{yMonths.length} {yMonths.length === 1 ? 'month' : 'months'} · D{yTotal.toLocaleString()}</span>
                    </button>
          ...
          {payConfirm && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => !payPosting && setPayConfirm(null)}>
              <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
                ...
                {payConfirm.loading ? (
                  <p className="text-sm text-[var(--color-ink-soft)] py-6 text-center">Checking Zoho…</p>
                ) : payConfirm.error ? (
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between border-t border-[var(--color-line-soft)] pt-2"><span className="text-[var(--color-ink)] font-semibold">Total to Books</span><span className="font-bold">D{payConfirm.preview.total.toLocaleString()}</span></div>
                ...
                <div className="flex justify-end gap-2 mt-6">
                  <button type="button" onClick={() => setPayConfirm(null)} disabled={payPosting} className="px-3 py-2 rounded-lg text-sm font-medium bg-[var(--color-fill)] text-[var(--color-ink-soft)] hover:bg-[var(--color-line)]">Cancel</button>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => !oneOff.busy && setOneOff(null)}>
              <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex justify-end gap-2 mt-5">
                  <button type="button" onClick={() => setOneOff(null)} disabled={oneOff.busy} className="px-4 py-2 rounded-lg text-sm bg-[var(--color-fill)] text-[var(--color-ink-soft)] hover:bg-[var(--color-line)]">Cancel</button>
          ...
          {bulk && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => !bulk.busy && setBulk(null)}>
              <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
                ...
                {!bulk.done ? (
                  <>
                    <p className="text-sm text-[var(--color-ink-soft)] mb-4">Each payment posts to Zoho Books with the numbers as entered, dated {payDate}. Anyone already paid in Books is skipped.</p>
                    <div className="space-y-1.5 text-sm max-h-56 overflow-y-auto">
                      ...
                      <div className="flex justify-between border-t border-[var(--color-line-soft)] pt-2 font-bold"><span>Total</span><span className="tabular-nums">D{bulk.people.reduce((s, p) => { const d = payDraft[p.name] || {}; return s + (Number(d.salary) || 0) + (Number(d.bonus) || 0); }, 0).toLocaleString()}</span></div>
                    </div>
                    <div className="flex justify-end gap-2 mt-5">
                      <button type="button" onClick={() => setBulk(null)} disabled={bulk.busy} className="px-4 py-2 rounded-lg text-sm bg-[var(--color-fill)] text-[var(--color-ink-soft)] hover:bg-[var(--color-line)]">Cancel</button>
                  ...
                  <>
                    <div className="space-y-1.5 text-sm mt-3 max-h-56 overflow-y-auto">
                      {(bulk.results || []).map(r => <div key={r.name} className="flex justify-between gap-3"><span className="text-[var(--color-ink-soft)]">{r.name}</span><span className={`text-right ${r.status.startsWith('paid') ? 'text-emerald-700' : 'text-amber-600'}`}>{r.status}</span></div>)}
                    </div>
                    <div className="flex justify-end mt-5">
                      <button type="button" onClick={() => setBulk(null)} className="px-4 py-2 rounded-lg text-sm text-white bg-[var(--color-ink)] hover:bg-[var(--color-ink)]">Done</button>
                    </div>
                  </>
                )}
              </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => !payPosting && setPayEdit(null)}>
              <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex items-center justify-between border-t border-[var(--color-line-soft)] pt-2"><span className="text-[var(--color-ink)] font-semibold">New total</span><span className="font-bold">D{((Number(payEdit.salary) || 0) + (Number(payEdit.bonus) || 0)).toLocaleString()}</span></div>
                  <label className="flex items-center justify-between gap-3"><span className="text-[var(--color-ink-soft)]">Paid via</span>
                ...
                <div className="flex justify-end gap-2 mt-6">
                  <button type="button" onClick={() => setPayEdit(null)} disabled={payPosting} className="px-3 py-2 rounded-lg text-sm font-medium bg-[var(--color-fill)] text-[var(--color-ink-soft)] hover:bg-[var(--color-line)]">Cancel</button>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => !payPosting && setPayUndo(null)}>
              <div className="bg-white rounded-lg shadow-xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex justify-end gap-2">
                  <button type="button" onClick={() => setPayUndo(null)} disabled={payPosting} className="px-3 py-2 rounded-lg text-sm font-medium bg-[var(--color-fill)] text-[var(--color-ink-soft)] hover:bg-[var(--color-line)]">Cancel</button>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg border border-[var(--color-line)] p-6">
            ...
            {allWarnings.length === 0 ? (
              <div className="p-12 text-center text-[var(--color-ink-faint)] text-sm">No warnings recorded.</div>
            ) : (
                  ...
                  return (
                    <div key={w.id} className="flex items-start gap-3 p-4 border border-[var(--color-line)] rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-[var(--color-ink-soft)] mt-0.5">{w.reason}</p>
      ...
      {tab === 'past' && (() => {
        ...
        <div className="bg-white rounded-lg border border-[var(--color-line)] p-6">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-5">
            {cats.map(c => (
              <button key={c} type="button" onClick={() => setPastFilter(c)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${pastFilter === c ? 'bg-[var(--color-ink)] text-white' : 'bg-[var(--color-fill)] text-[var(--color-ink-soft)] hover:bg-[var(--color-line)]'}`}>
            ...
            {shown.map((p, i) => (
              <div key={i} onClick={() => navigate(`/past/${p.name.toLowerCase().replace(/\s+/g, '-')}`)} className="p-4 border border-[var(--color-line)] rounded-lg cursor-pointer hover:border-[var(--color-line)] transition-all">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  ...
                  <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    {restorableMap[p.name.toLowerCase().replace(/\s+/g, '')] && (
                      <button onClick={() => restorePast(restorableMap[p.name.toLowerCase().replace(/\s+/g, '')])} className="flex items-center gap-1 rounded-full border border-[var(--color-line)] px-3 py-1 text-xs font-semibold text-[var(--color-ink-soft)] hover:border-emerald-500 hover:text-emerald-600"><RotateCcw size={12} /> Restore</button>
                    )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 pt-3 border-t border-[var(--color-line-soft)]">
                  <div><p className="text-[10px] uppercase tracking-wider font-bold text-[var(--color-ink-faint)]">Left</p><p className="text-sm text-[var(--color-ink)] mt-0.5">{p.date || '—'}</p></div>
                  <div><p className="text-[10px] uppercase tracking-wider font-bold text-[var(--color-ink-faint)]">Monthly pay</p><p className="text-sm text-[var(--color-ink)] mt-0.5">D{(p.pay || 0).toLocaleString()}</p></div>
                  <div><p className="text-[10px] uppercase tracking-wider font-bold text-[var(--color-ink-faint)]">Final settlement</p><p className="text-sm text-[var(--color-ink)] mt-0.5">{p.finalPay > 0 ? `D${p.finalPay.toLocaleString()}` : '—'}</p></div>
                  <div className="col-span-2 sm:col-span-1"><p className="text-[10px] uppercase tracking-wider font-bold text-[var(--color-ink-faint)]">Reason</p><p className="text-sm text-[var(--color-ink-soft)] mt-0.5">{p.reason}</p></div>
                </div>
              </div>
            ))}
            {shown.length === 0 && <div className="p-10 text-center text-[var(--color-ink-faint)] text-sm">No one in this category.</div>}
          </div>
        </div>
        );
      })()}
