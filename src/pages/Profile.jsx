...
function Stat({ label, value, accent }) {
  return (
    <div>
      ...
      <p className={`text-sm font-semibold ${accent || 'text-[var(--color-ink)]'}`}>{value}</p>
    </div>
  )
}
        ...
        <div>
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between mb-5">
              <span className="text-sm font-semibold text-[var(--color-ink)]">{c.type || 'Contract'}</span>
            ...
            <ol className="relative space-y-5 pl-6">
              ...
              {history.slice().reverse().map((h, i) => (
                <li key={i} className="relative">
                  ...
                  <div className="text-xs text-[var(--color-ink-faint)]">{fmtDate(h.date)}{h.dateApproximate ? ' (approx.)' : ''}</div>
                </li>
              ))}
            </ol>
          </Card>
