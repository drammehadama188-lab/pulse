    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <div className={`${CARD} p-5`}>
        ...
        {perf.averageReview == null ? (
          ...
          <>
            ...
            <p className="mt-1.5 text-[12px] text-[var(--color-ink-faint)]">Average across {scored.length} review{scored.length === 1 ? '' : 's'}</p>
            {ratings && (
              <div className="mt-4 divide-y divide-[var(--color-line-soft)]">
                {Object.entries(ratings).map(([k, v]) => (
                  <div key={k} className="flex items-center justify-between py-2.5">
                    <span className="text-[13px] text-[var(--color-ink-soft)]">{k}</span>
                    <span className="text-[13px] font-medium text-[var(--color-ink)]">{v}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      <div className={`${CARD} p-5`}>
        ...
        <div className="space-y-3.5">
          {perf.score != null && <Bar label="Review score" pct={perf.score} colour="var(--color-pill-active)" />}
          {perf.sales?.target ? (
            <Bar label={`Sales · ${perf.sales.actual ?? 0} of ${perf.sales.target}`}
          ...
          {perf.reviews?.map((r) => (
            <div key={r.period} className="flex items-center justify-between gap-3 py-3">
              <span className="min-w-0">
                <span className="block text-[13px] font-medium text-[var(--color-ink)]">{r.period} review</span>
                {r.notes && <span className="block truncate text-[12px] text-[var(--color-ink-faint)]">{r.notes}</span>}
              </span>
              <span className="flex shrink-0 items-center gap-3">
                {r.status && <span className="text-[12px] text-[var(--color-ink-faint)]">{r.status}</span>}
                {typeof r.score === 'number' && (
                  <span className="inline-flex items-center gap-1 text-[13px] font-semibold text-[var(--color-ink)]">
                    <Star size={13} className="text-[var(--color-pill-leave)]" /> {r.score}
                  </span>
                )}
              </span>
            </div>
