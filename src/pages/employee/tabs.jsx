...
import {
  band, defaultPeriod, effectiveScore, insightsFor, periodLabel, salesAttainment,
  salesForPeriod, trendSeries, CUR_PERIOD,
...
// ── Performance ────────────────────────────────────────────────────
// 🔒 Reads the SAME endpoints and the SAME helpers as the Performance page
// (src/lib/performance.js). It was summarising its own version before, which
// is how Kaddy's record came out empty while her Performance page was full:
// a sales role's headline score is sales ATTAINMENT when no manual score is
// set, and that fallback lived in the helper I was not calling.
export function Performance({ person, username }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api(`/employee-profile?name=${n}`).then((d) => {
        const pr = d.profile || {};
        const raw = pr.performanceScore;
        setLive({ score: raw === '' || raw == null ? null : Number(raw), note: pr.performanceNote || '', nextReview: pr.nextReview || '' });
      }).catch(() => {}),
  ...
  // The roster shape the helpers expect: department is `type`, and a sales
  // target makes someone a sales role.
  const asPerson = { name: person.name, type: person.department, target: sales?.monthlyTarget || 0 };
  const { score, source } = effectiveScore(live.score, asPerson, sales, period);
  const attainment = salesAttainment(sales, period);
  const periodSales = salesForPeriod(sales, period);
  const insights = insightsFor(person.name, { [person.name]: live }, { [person.name]: reviews }, sales);
  const series = trendSeries(reviews, live.score);
  const tone = score == null ? null : band(score);

  if (!loaded) return <p className="text-[13px] text-[var(--color-ink-soft)]">Loading…</p>;

  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <div className={`${CARD} p-5`}>
        <CardHead title={`Score · ${periodLabel(period.period || CUR_PERIOD)}`} action={
          <Link to={`/performance/${username}`} className="text-[13px] font-medium text-[var(--color-brand)] hover:underline">Open performance</Link>} />
        {score == null ? (
          <Nothing to={`/performance/${username}`} cta="Set a score">
            No score for this period. A manager can set one, or a sales target will produce one from real sales.
          </Nothing>
        ) : (
          <>
            <p className="text-[34px] font-semibold leading-none" style={{ color: tone?.color || 'var(--color-ink)' }}>
              {score}<span className="text-[16px] font-normal text-[var(--color-ink-faint)]">%</span>
            </p>
            <p className="mt-1.5 text-[12px] text-[var(--color-ink-faint)]">
              {tone?.label}{source === 'sales' ? ' · from sales attainment' : source === 'manual' ? ' · set by a manager' : ''}
            </p>
            {live.note && <p className="mt-3 rounded-[8px] bg-[var(--color-soft)] p-3 text-[13px] text-[var(--color-ink)]">{live.note}</p>}
            {live.nextReview && <p className="mt-3 text-[12px] text-[var(--color-ink-faint)]">Next review {day(live.nextReview)}</p>}
          </>
        )}
      </div>
      ...
      <div className={`${CARD} p-5`}>
        ...
        {!periodSales?.target ? (
          <Nothing>No sales target for this period, so there is nothing to measure against.</Nothing>
        ) : (
          <>
            <div className="flex items-baseline justify-between gap-3">
              <span className="text-[13px] text-[var(--color-ink-soft)]">{periodSales.sales ?? 0} of {periodSales.target} sold</span>
              <span className="text-[15px] font-semibold text-[var(--color-ink)]">{attainment}%</span>
            </div>
            <div className="mt-2 h-1.5 rounded-full bg-[var(--color-line-soft)]">
              <div className="h-full rounded-full bg-[var(--color-stage-new)]" style={{ width: `${Math.min(100, attainment || 0)}%` }} />
            </div>
            {series.length > 1 && (
              <p className="mt-3 text-[12px] text-[var(--color-ink-faint)]">
                {series.length} scored period{series.length === 1 ? '' : 's'} on record.
              </p>
            )}
          </>
        )}
          ...
          <ul className="space-y-2">
            {insights.map((it, i) => (
              <li key={i} className="flex items-start gap-2.5 text-[13px]"
                style={{ color: it.tone === 'bad' ? 'var(--color-stage-out)' : it.tone === 'good' ? 'var(--color-pill-active)' : 'var(--color-ink-soft)' }}>
                <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-current" /> {it.text}
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className={`${CARD} p-5`}>
        ...
        {reviews.length === 0 ? (
          <Nothing to={`/performance/${username}`} cta="Record a review">
            No monthly review has been locked in yet.
          </Nothing>
        ) : (
          <div className="divide-y divide-[var(--color-line-soft)]">
            {reviews.slice(0, 6).map((r) => (
              <div key={r.period} className="flex items-center justify-between gap-3 py-3">
                <span className="min-w-0">
                  <span className="block text-[13px] font-medium text-[var(--color-ink)]">{periodLabel(r.period)}</span>
                  {r.notes && <span className="block truncate text-[12px] text-[var(--color-ink-faint)]">{r.notes}</span>}
                </span>
                {r.score != null && (
                  <span className="inline-flex items-center gap-1 text-[13px] font-semibold" style={{ color: band(Number(r.score)).color }}>
      ...
      <div className={`${CARD} p-5`}>
        <CardHead title="Warnings" action={<Link to="/reviews" className="text-[13px] font-medium text-[var(--color-brand)] hover:underline">Records</Link>} />
        {warnings.length === 0 ? <Nothing>No warnings on file.</Nothing> : (
          <div className="divide-y divide-[var(--color-line-soft)]">
            {warnings.map((w, i) => (
              <div key={i} className="py-3">
                <p className="flex items-center gap-2 text-[13px] font-medium text-[var(--color-ink)]">
                  <AlertTriangle size={13} className="text-[var(--color-stage-out)]" /> {w.type} warning
                </p>
                <p className="mt-1 text-[12px] text-[var(--color-ink-faint)]">{day(w.date)}</p>
                {w.reason && <p className="mt-1.5 text-[13px] text-[var(--color-ink-soft)]">{w.reason}</p>}
              </div>
            ))}
          </div>
        )}
