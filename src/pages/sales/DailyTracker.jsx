    ...
    <div className="space-y-7">
      ...
      <Card className="p-5 sm:p-5" >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-base font-semibold text-[var(--color-ink)]">Today</h2>
          {today.checkIn ? (
        ...
        {today.notes > 0 && <div className="mt-3 text-[13px] text-[var(--color-ink-faint)]"><StickyNote size={13} className="mr-1 inline" />{today.notes} note{today.notes > 1 ? 's' : ''} added</div>}
      </Card>

      {/* recent days */}
      <div>
        <h2 className="mb-3 text-base font-semibold text-[var(--color-ink)]">Recent days</h2>
        {past.length === 0 ? (
            ...
            {past.map((d) => (
              <div key={d.key} className="flex flex-wrap items-center gap-x-4 gap-y-1 px-5 py-3.5">
                <div className="w-32 shrink-0">
                  ...
                  {d.checkIn && <div className="text-[11.5px] text-[var(--color-ink-faint)]"><LogIn size={11} className="mr-1 inline" />{timeShort(d.checkIn)}</div>}
                </div>
                <div className="flex flex-1 flex-wrap gap-x-4 gap-y-1 text-[13px] text-[var(--color-ink-soft)]">
...
function Metric({ icon: Icon, tone, label, value, sub }) {
  return (
    <div className="rounded-lg bg-[var(--color-fill)] p-4">
      ...
      <div className="mt-1.5 text-[22px] font-semibold tracking-tight text-[var(--color-ink)]">{value}</div>
      {sub && <div className="text-[11.5px] text-[var(--color-good)]">{sub}</div>}
    </div>
  )
}
