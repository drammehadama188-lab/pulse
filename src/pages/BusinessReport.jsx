...
export default function BusinessReport() {
  ...
  return (
    <div className="max-w-4xl space-y-7">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-[var(--color-ink)]">Business report</h1>
      ...
      {data.flags.length > 0 && (
        <Card className="border-l-4 border-[var(--color-bad)] p-4">
          <h2 className="text-sm font-extrabold uppercase tracking-wide text-[var(--color-bad)]">Needs your attention</h2>
          ...
          {data.goals.map((g) => {
            ...
            return (
              <div key={g.title} className="flex items-center gap-4 px-4 py-3 text-sm">
                ...
                <span className="tabular-nums text-[var(--color-ink-soft)]"><span className="text-lg font-extrabold text-[var(--color-ink)]">{g.actual ?? '—'}</span>{g.unit === '%' ? '%' : ''} {g.target != null ? `of ${g.target}${g.unit === '%' ? '%' : ''}` : ''} <span className="text-xs">{g.unit !== '%' ? g.unit : ''}</span></span>
      ...
      {/* money */}
      <section>
        <h2 className="mb-2 text-sm font-extrabold uppercase tracking-wide text-[var(--color-ink-faint)]">Money</h2>
        <div className="grid gap-3 sm:grid-cols-3">
          <Card className="p-4">
            <div className="text-[10px] font-bold uppercase tracking-wide text-[var(--color-ink-faint)]">Payroll paid</div>
            <div className="mt-1 text-2xl font-extrabold tabular-nums text-[var(--color-ink)]">{dalasi(data.money.payrollCost)}</div>
          ...
          <Card className="p-4">
            ...
            <div className="mt-1 text-2xl font-extrabold tabular-nums text-[var(--color-good)]">{data.money.renewalRevenue != null ? dalasi(data.money.renewalRevenue) : '—'}</div>
          ...
          <Card className="p-4">
            ...
            <div className="mt-1 text-2xl font-extrabold tabular-nums text-[var(--color-bad)]">{data.money.renewalOutstanding != null ? dalasi(data.money.renewalOutstanding) : '—'}</div>
      ...
      {/* team */}
      <section>
        <h2 className="mb-2 text-sm font-extrabold uppercase tracking-wide text-[var(--color-ink-faint)]">Team</h2>
        <Card className="flex flex-wrap gap-x-8 gap-y-2 p-4">
          <div>
            ...
            <div className="text-xl font-extrabold tabular-nums text-[var(--color-ink)]">{data.team.headcount}</div>
          </div>
          <div>
            <div className="text-[10px] font-bold uppercase tracking-wide text-[var(--color-ink-faint)]">Attendance</div>
            <div className="text-xl font-extrabold tabular-nums text-[var(--color-ink)]">{data.team.attendancePct != null ? `${data.team.attendancePct}%` : '—'}</div>
          ...
          <div>
            ...
            <div className="text-xl font-extrabold tabular-nums text-[var(--color-ink)]">{data.team.late}</div>
          </div>
        </Card>
      </section>
