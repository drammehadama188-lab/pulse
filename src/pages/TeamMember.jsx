...
export default function TeamMember() {
  ...
  return (
    <div className="space-y-4">
      <button onClick={() => navigate('/team-dashboard')} className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-[var(--color-ink-soft)] hover:text-[var(--color-brand)]">
      ...
      <div>
        ...
        {kpis.length === 0 ? (
          <Card className="p-5 text-center text-[13px] text-[var(--color-ink-faint)]">No scorecard for this role yet.</Card>
        ) : (
              ...
              return (
                <div key={k.key} className="px-4 py-3.5 sm:px-5">
                  <div className="flex items-center justify-between gap-3">
                    <div className="text-[13px] font-semibold text-[var(--color-ink)]">{k.label}<span className="ml-2 text-[11.5px] font-medium text-[var(--color-ink-faint)]">weight {k.weight}%</span></div>
                    <div className="text-[13px] font-semibold tabular-nums text-[var(--color-ink)]">
                      {k.actual == null
                        ? <span className="text-[var(--color-ink-faint)]">— <span className="text-[11.5px] font-medium">Connecting to Admin</span></span>
                        : <>{k.actual}{k.kind === 'percent' ? '%' : ''} <span className="text-[11.5px] font-medium text-[var(--color-ink-faint)]">/ {k.target}{k.kind === 'percent' ? '%' : ` ${k.unit}`}</span></>}
                    </div>
                  </div>
        ...
        <p className="mt-2 text-[11.5px] text-[var(--color-ink-faint)]">Weak KPIs (below target) are where a check-in helps most. Actuals fill in as the Admin feed connects and reviews are locked.</p>
      </div>

      {/* Log a check-in */}
      <div>
        <SectionTitle>Log coaching / check-in</SectionTitle>
        <p className="mb-3 -mt-1 text-[11.5px] text-[var(--color-ink-faint)]">
        ...
        {isViewAs
          ? <Card className="p-5 text-[13px] font-medium text-[var(--color-ink-faint)]">Read-only view — logging is disabled while viewing as someone else.</Card>
      ...
      <div>
        ...
        {data.coaching.length === 0 ? (
          <Card className="p-5 text-center text-[13px] text-[var(--color-ink-faint)]">No check-ins logged yet.</Card>
        ) : (
          <Card className="divide-y divide-[var(--color-line-soft)] overflow-hidden p-0">
            {data.coaching.map((c) => {
              ...
              return (
                <div key={c.id} className="flex gap-3 px-4 py-3.5 sm:px-5">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-[13px] font-semibold text-[var(--color-ink)]">{c.title || T.label}</div>
                  ...
                  {(canEditCoaching || canDeleteCoaching) && (
                    <div className="flex shrink-0 items-start gap-1">
                      ...
                      {canDeleteCoaching && (confirmDel === c.id ? (
                        <button onClick={() => removeCoaching(c.id)} disabled={delBusy} className="rounded-lg bg-[var(--color-bad)] px-2 py-1 text-[11.5px] font-semibold text-white disabled:opacity-60">{delBusy ? 'Deleting…' : 'Delete?'}</button>
                      ) : (
...
function CoachingForm({ username, name, record = null, onSaved, onCancel }) {
  ...
  return (
    <Card className="space-y-4 p-5">
      ...
      {error && <div className="text-[13px] font-medium text-[var(--color-bad)]">{error}</div>}
