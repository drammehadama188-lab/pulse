      ...
      <div className={`${CARD} p-5`}>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="text-base font-semibold text-[var(--color-ink)]">{p.title}</h3>
            <p className="text-[12px] text-[var(--color-ink-faint)] mt-0.5">{[p.department, p.location, p.employment].filter(Boolean).join(' · ')}</p>
          </div>
        ...
        {p.summary && <p className="mt-3 text-[13px] text-[var(--color-ink-soft)] whitespace-pre-wrap">{p.summary}</p>}
        ...
        <div className="mt-4 flex items-center justify-between gap-2">
          <Link to={`/recruitment/applicants?position=${p.id}&stage=all`} className="text-[13px] font-medium text-[var(--color-ink)] hover:underline">See applicants</Link>
          <button onClick={() => setStatus(p, p.status === 'open' ? 'closed' : 'open')} className="text-[13px] font-medium text-[var(--color-ink-soft)] hover:text-[var(--color-ink)]">
  ...
  return (
    <div>
      <PageHead title="Positions" count={positions.length || null}>
        <button onClick={() => { setForm(BLANK); setError(null); setEditing('new'); }} className={BTN_PRIMARY}><Plus size={16} /> New position</button>
      </PageHead>
        ...
        : positions.length === 0 ? <div className={`${CARD} p-12 text-center text-[var(--color-ink-faint)] text-[13px]`}>No positions yet. Add the job you are hiring for and applicants file under it.</div>
          : (
            <div className="space-y-6">
              ...
              {closed.length > 0 && (
                <div>
                  <h2 className="text-[13px] font-semibold text-[var(--color-ink-faint)] mb-3">Closed</h2>
            ...
            <div className="space-y-3">
              {[['title', 'Title *'], ['department', 'Department'], ['location', 'Location']].map(([k, label]) => (
                <label key={k} className="block">
                  ...
                  <input value={form[k] || ''} onChange={e => setForm(f => ({ ...f, [k]: e.target.value }))} className="mt-1 w-full border border-[var(--color-line)] rounded-lg px-3 py-2 text-[13px]" />
                </label>
              ))}
              <div className="grid grid-cols-2 gap-3">
                <label className="block">
                  <span className="text-[11.5px] font-medium text-[var(--color-ink-faint)]">Type</span>
                  <select value={form.employment} onChange={e => setForm(f => ({ ...f, employment: e.target.value }))} className="mt-1 w-full border border-[var(--color-line)] rounded-lg px-3 py-2 text-[13px] bg-white">
                    {EMPLOYMENT.map(x => <option key={x}>{x}</option>)}
                  </select>
                </label>
                <label className="block">
                  <span className="text-[11.5px] font-medium text-[var(--color-ink-faint)]">Openings</span>
                  <input type="number" min={1} value={form.openings} onChange={e => setForm(f => ({ ...f, openings: e.target.value }))} className="mt-1 w-full border border-[var(--color-line)] rounded-lg px-3 py-2 text-[13px]" />
                </label>
              </div>
              <label className="block">
                <span className="text-[11.5px] font-medium text-[var(--color-ink-faint)]">What the job is</span>
                <textarea value={form.summary || ''} onChange={e => setForm(f => ({ ...f, summary: e.target.value }))} rows={3} className="mt-1 w-full border border-[var(--color-line)] rounded-lg px-3 py-2 text-[13px]" />
              </label>
            </div>
            <div className="flex justify-end gap-2 mt-5">
              <button onClick={() => setEditing(null)} disabled={saving} className="px-3 py-2 rounded-lg text-[13px] bg-[var(--color-fill)] text-[var(--color-ink-soft)] hover:bg-[var(--color-line)]">Cancel</button>
